...
export class ReportesService {
  ...
  async getReporteVentas(boticaId: string, query: QueryReportesDto) {
    this.logger.log(
      `Generando reporte de ventas para sucursal: ${query.sucursal_id || 'Global'}`,
    );
    ...
    const ticketPromedio =
      cantidadTransacciones > 0 ? totalVentas / cantidadTransacciones : 0;
    ...
    ventas.forEach((v) => {
      v.detalles_ventas.forEach((d) => {
        const precioCompraUnitario = Number(
          d.lotes?.precio_compra_unidad_base || 0,
        );
        const cantidadBase =
          d.cantidad *
        ...
        const costoLinea =
          precioCompraUnitario > 0
            ? precioCompraUnitario * cantidadBase
            : Number(d.subtotal) * 0.65;
        ...
        const prodNombre =
          ...
          'Producto';
        const presNombre =
          (d.productos_presentaciones as any)?.presentacion_nombre || 'Unidad';
        const prev = productosVendidosMap.get(prodId) || {
          nombre: prodNombre,
    ...
    const margenBrutoPct =
      subtotalBase > 0
        ? Number(((gananciaBrutaReal / subtotalBase) * 100).toFixed(1))
        : 0;
    ...
    ventas.forEach((v) => {
      v.pagos.forEach((p) => {
        ...
        pagosMap.set(
          nombreMetodo,
          (pagosMap.get(nombreMetodo) || 0) + Number(p.monto),
        );
      });
    });

    const desgloseMetodosPago = Array.from(pagosMap.entries()).map(
      ([metodo, monto]) => ({
        metodo,
        monto,
        porcentaje:
          totalVentas > 0
            ? Number(((monto / totalVentas) * 100).toFixed(1))
            : 0,
      }),
    );
    ...
    const ventasPorDiaMap = new Map<
      string,
      { fecha: string; total: number; transacciones: number }
    >();
    ventas.forEach((v) => {
      const fechaKey = new Date(v.fecha).toISOString().slice(0, 10);
      const prev = ventasPorDiaMap.get(fechaKey) || {
        fecha: fechaKey,
        total: 0,
        transacciones: 0,
      };
    ...
    const tendenciasDiarias = Array.from(ventasPorDiaMap.values()).sort(
    ...
    const listaVentas = ventas.map((v) => {
      ...
      const itemsDetalle = v.detalles_ventas.map((d) => {
        ...
        const presNombre =
          (d.productos_presentaciones as any)?.presentacion_nombre ||
          (d.productos_presentaciones as any)?.unidades_presentacion?.nombre ||
          'Unidad';
        return {
  ...
  async getReporteInventario(query: QueryReportesDto) {
    ...
    const valorTotalInventarioCostos = lotes.reduce(
      (acc, l) =>
    ...
    const lotesStockCriticoCount = lotes.filter(
      (l) => l.stock_actual > 0 && l.stock_actual <= 15,
    ).length;
    ...
    const lotesLista = lotes.map((l) => ({
      ...
      producto_nombre:
        l.productos_comerciales?.nombre_comercial || 'Producto sin Nombre',
  ...
  /** Vista administrativa: explica ventas, costos, gastos y capital inmovilizado. */
  async getReporteFinanciero(boticaId: string, query: QueryReportesDto) {
    const inicio = query.fecha_inicio
      ? new Date(`${query.fecha_inicio}T00:00:00.000Z`)
      : new Date(new Date().getFullYear(), new Date().getMonth(), 1);
    const fin = query.fecha_fin
  ...
  async generarLibroVentasPLE(query: QueryReportesDto) {
    ...
    const lineasPLE = ventas.map((v, index) => {
      const fechaStr = new Date(v.fecha)
        .toISOString()
        .slice(0, 10)
        .replace(/-/g, '');
      const periodo = new Date(v.fecha)
        .toISOString()
        .slice(0, 7)
        .replace('-', '');
