...
export class XmlBuilderService {
  buildInvoice(dto: CreateInvoiceDto): string {
    ...
    const customer = root
      ...
      .ele('cac:PartyIdentification')
      .ele('cbc:ID', {
        schemeID: dto.cliente.tipoDocumento,
        schemeName: 'Documento de Identidad',
        schemeAgencyName: 'PE:SUNAT',
        schemeURI: 'urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo06',
      })
      .txt(dto.cliente.numeroDocumento)
      .up()
      .up()
      .ele('cac:PartyLegalEntity')
      .ele('cbc:RegistrationName')
      .txt(dto.cliente.razonSocial)
      .up();
    ...
    const taxTotal = root
      .ele('cac:TaxTotal')
      .ele('cbc:TaxAmount', { currencyID: dto.moneda })
      .txt(dto.totalIgv.toFixed(2))
      .up()
      .ele('cac:TaxSubtotal')
      .ele('cbc:TaxableAmount', { currencyID: dto.moneda })
      .txt(dto.totalGravadas.toFixed(2))
      .up()
      .ele('cbc:TaxAmount', { currencyID: dto.moneda })
      .txt(dto.totalIgv.toFixed(2))
      .up()
    ...
    root
      ...
      .ele('cbc:LineExtensionAmount', { currencyID: dto.moneda })
      .txt(dto.totalGravadas.toFixed(2))
      .up()
      .ele('cbc:TaxInclusiveAmount', { currencyID: dto.moneda })
      .txt(dto.importeTotal.toFixed(2))
      .up()
      .ele('cbc:PayableAmount', { currencyID: dto.moneda })
      .txt(dto.importeTotal.toFixed(2))
      .up()
      .up();
    ...
    dto.items.forEach((item, index) => {
      const invoiceLine = root
        ...
        .up()
        .ele('cbc:InvoicedQuantity', { unitCode: item.unidadMedida })
        ...
        .ele('cbc:PriceAmount', { currencyID: dto.moneda })
        .txt(item.precioUnitario.toFixed(2))
        .up()
        .ele('cbc:PriceTypeCode')
        .txt('01')
        .up() // 01 = Precio Unitario (Incluye IGV)
        .up()
        .up()
