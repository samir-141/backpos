-- Productos generales pueden manejar existencias sin fecha de vencimiento.
...
SELECT
    ...
    COALESCE((SELECT SUM(l.stock_actual) FROM public.lotes l WHERE l.producto_comercial_id = pc.id AND l.deleted_at IS NULL AND l.stock_actual > 0 AND (pc.requiere_vencimiento = false OR l.fecha_vencimiento >= CURRENT_DATE)), 0)::int AS stock_total,
    (SELECT MIN(l.fecha_vencimiento) FROM public.lotes l WHERE l.producto_comercial_id = pc.id AND l.deleted_at IS NULL AND l.stock_actual > 0 AND (pc.requiere_vencimiento = false OR l.fecha_vencimiento >= CURRENT_DATE)) AS fecha_vencimiento_proximo,
    pc.created_at, pc.updated_at, pc.deleted_at,
    (SELECT l.numero_lote FROM public.lotes l WHERE l.producto_comercial_id = pc.id AND l.deleted_at IS NULL AND l.stock_actual > 0 AND (pc.requiere_vencimiento = false OR l.fecha_vencimiento >= CURRENT_DATE) ORDER BY l.fecha_vencimiento ASC NULLS LAST, l.fecha_ingreso ASC NULLS LAST LIMIT 1) AS lote_fefo_numero,
    (SELECT l.fecha_vencimiento FROM public.lotes l WHERE l.producto_comercial_id = pc.id AND l.deleted_at IS NULL AND l.stock_actual > 0 AND (pc.requiere_vencimiento = false OR l.fecha_vencimiento >= CURRENT_DATE) ORDER BY l.fecha_vencimiento ASC NULLS LAST, l.fecha_ingreso ASC NULLS LAST LIMIT 1) AS lote_fefo_vencimiento,
