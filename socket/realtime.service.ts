...
export class RealtimeService {
  ...
  notificarVentaCreada(sucursalId: string, ventaPayload: any) {
    this.logger.log(`[Realtime] Notificando venta.creada en sucursal ${sucursalId}`);
    this.eventsGateway.emitirASucursal(sucursalId, 'venta.creada', ventaPayload);
    this.eventsGateway.emitirGlobal('dashboard.actualizado', { sucursal_id: sucursalId });
  }
  ...
  notificarVentaAnulada(sucursalId: string, ventaId: string) {
    ...
    this.eventsGateway.emitirASucursal(sucursalId, 'venta.anulada', { venta_id: ventaId });
    this.eventsGateway.emitirGlobal('dashboard.actualizado', { sucursal_id: sucursalId });
  }
   ...
   */
  notificarStockActualizado(sucursalId: string, productoComercialId: string, nuevoStockTotal: number) {
    this.logger.log(`[Realtime] Notificando stock.actualizado para ${productoComercialId}: ${nuevoStockTotal}`);
   ...
   */
  notificarAlertaStock(sucursalId: string, productoNombre: string, stockActual: number) {
   ...
   */
  notificarCajaAperturada(sucursalId: string, cajaId: string, usuarioId: string, montoInicial: number) {
   ...
   */
  notificarProductoCambio(sucursalId: string, accion: 'creado' | 'actualizado' | 'eliminado', producto: any) {
    this.logger.log(`[Realtime] Notificando producto.${accion}`);
    this.eventsGateway.emitirASucursal(sucursalId, `producto.${accion}`, producto);
  }

  /**
   * Notifica una alerta o mensaje al sistema
   */
  notificarGeneral(payload: { titulo: string; mensaje: string; tipo: 'INFO' | 'SUCCESS' | 'WARNING' | 'DANGER'; sucursalId?: string }) {
    if (payload.sucursalId) {
      this.eventsGateway.emitirASucursal(payload.sucursalId, 'notification.created', payload);
    } else {
