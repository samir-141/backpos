...
export class CatalogosService {
  ...
  async findOne(
    ...
    const rows = await this.prisma.queryRaw(
      `SELECT * FROM public.${config.tabla} WHERE id = $1::uuid AND deleted_at IS NULL AND botica_id = $2::uuid`,
      [id, boticaId],
    );
  ...
  async update(
    ...
    await this.findOne(boticaId, tipo, id);
    ...
    if (
      config.camposEspeciales.includes('descripcion') &&
    ...
    if (
      config.camposEspeciales.includes('telefono') &&
  ...
  async remove(
    ...
    try {
      ...
      return {
        mensaje:
          result[0]?.mensaje ||
          `Item eliminado correctamente de ${config.tabla}`,
      };
    } catch (error: any) {
