...
export class VentasService {
  ...
  async create(
    ...
    try {
      const resultado = await this.prisma.$transaction(
        async (tx) => {
          ...
          if (dto.datos_cliente && dto.datos_cliente.numero_documento) {
            const numDoc = String(dto.datos_cliente.numero_documento).trim();
            ...
            if (!cliente) {
              cliente = await tx.clientes.create({
                data: {
                  ...
                  numero_documento: numDoc,
                  nombre:
                    dto.datos_cliente.nombre_razon_social || 'CLIENTE POS',
              ...
              cliente = await tx.clientes.update({
                ...
                data: {
                  deleted_at: null,
                  nombre:
                    dto.datos_cliente.nombre_razon_social || cliente.nombre,
                },
              });
            }
          ...
          for (const item of dto.items) {
            ...
            const precioUnitarioCents = this.toCents(
              presentacion.precio_actual,
            );
            ...
            const afectoIgv =
              presentacion.productos_comerciales.medicamentos?.afecto_igv ??
              true;
          ...
          if (!tipoMovVenta) {
            try {
              ...
              tipoMovVenta = await tx.tipos_movimientos_inventario.findFirst({
          ...
          if (!tipoMovVenta) {
            throw new BadRequestException(
              'No se pudo inicializar el tipo de movimiento de venta en el inventario.',
            );
          }
          ...
          if (!metodoPago) {
            metodoPago = await tx.metodos_pago.findFirst({
              where: { botica_id: boticaId, deleted_at: null },
            });
          }
          ...
          } else if (metodoPago.deleted_at) {
            metodoPago = await tx.metodos_pago.update({
          ...
          if (!metodoPago) {
            throw new BadRequestException(
              `No se pudo registrar ni asociar el método de pago ${metodoPagoNombre}.`,
            );
          }
      ...
      const { _meta, ...resVenta } = resultado;

      if (_meta) {
        // Audit Log (Sección 23 Documento 02)
        await this.auditService.registrar({
          usuario_id: _meta.finalUsuarioId,
          accion: 'VENTA_CREADA',
          tabla: 'ventas',
          botica_id: boticaId,
          observacion: `Venta registrada por S/ ${_meta.totalCalculado} - ${dto.tipo_comprobante} (${_meta.metodoPagoNombre})`,
        });

        // WebSocket Real-time Event (Sección 22 Documento 02)
        this.eventsGateway.emitirEvento('venta.creada', {
          venta_id: resVenta.venta_id,
          sucursal_id: _meta.finalSucursalId,
          total: _meta.totalCalculado,
        });
        ...
        if (asignaciones.length > 0) {
          ...
          if (consumos.length > 0) {
            ...
            for (const detalle of venta.detalles_ventas) {
              ...
              if (!equivalencia) {
                const presentacion =
                  await tx.productos_presentaciones.findFirst({
                    where: {
